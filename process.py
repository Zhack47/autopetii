...
from gcn_ref import gcn_inference2
...
class Autopet_baseline():  # SegmentationAlgorithm is not inherited in this class anymore
    ...
    def predict(self):
        ...
        trained_model_path = join(network_training_output_dir,  "Dataset510_AUTOPETII", "nnUNetTrainer__nnUNetPlans__3d_fullres")

        predictor = nnUNetPredictor(tile_step_size=0.5, use_mirroring=True, verbose=True, verbose_preprocessing=True, allow_tqdm=True)
        predictor.initialize_from_trained_model_folder(trained_model_path, use_folds=(0, 1, 2, 3), checkpoint_name="checkpoint_final.pth")
        ...
        #print("Segmentaion done, starting refinement")
        #gcn_inference2(gcn.load(),f"{output_file_trunc}.mha", f"{output_file_trunc}.npz", pet_mha)
